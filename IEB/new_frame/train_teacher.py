...
def train_teacher():
    ...
    teacher = IEBTeacherMagnitude(
        ...
        patch_len=16,  # ⚠️ 这个参数现在无用了,但保留兼容性
        stride=8,  # ⚠️ 这个参数现在无用了,但保留兼容性
    ...
    #     # CNN编码器
    #     {'params': teacher.cnn_encoder.parameters(),
    #      'lr': args.lr_cnn,
    #      'weight_decay': args.wd_cnn},
    #     # CNN投影层
    #     {'params': teacher.cnn_projection.parameters(),
    #      'lr': args.lr_projection,
    #      'weight_decay': args.wd_projection},
    #
    #     # ✅ 新增: GPT-2最后2层
    #     {'params': [p for n, p in teacher.llm_model.named_parameters()
    #                 if p.requires_grad and ('h.10' in n or 'h.11' in n)],
    #      'lr': 1e-6,  # 极小学习率
    #      'weight_decay': 1e-5},
    #
    #     # 分类头
    #     {'params': teacher.projection.parameters(),
    #      'lr': args.lr_classifier,
    #      'weight_decay': args.wd_classifier},
    #     # 其他可训练参数
    #     {'params': [p for n, p in teacher.named_parameters()
    #                 if p.requires_grad and
    #                 'cnn_encoder' not in n and
    #                 'cnn_projection' not in n and
    #                 'projection' not in n],
    #      'lr': args.lr_classifier,
    #      'weight_decay': args.wd_classifier}
    # ])
    ...
    #     lr=3e-4,            # lr=5e-4,weight_decay=1e-4,30epochs82.36%,dropout=0.3
    #     weight_decay=8e-4   # lr=3e-4,weight_decay=8e-4,30epochs87.92%,dropout=0.4
    # )                       # lr=3e-4,weight_decay=5e-4,30epochs75.97%,dropout=0.4
    #                         # lr=3e-4,weight_decay=8e-4,  30epochs  ?,dropout=?
    #
    # # 学习率调度器
    # scheduler = optim.lr_scheduler.CosineAnnealingLR(
    ...
    for epoch in range(1, epochs + 1):
        ...
        #         break
            ...
            # 删除 pbar.set_postfix，这是导致重复输出的根源！

        # pbar = tqdm(train_loader, desc=f'Epoch {epoch}/{epochs}',mininterval=5.0,      # 每2秒更新一次（而不是每个batch）
        #     ncols=50,            # 固定宽度100字符
        #     leave=True)
        # for batch in pbar:
        #     pressure = batch['pressure'].to(device)
