"""
Evaluation script for CMAD models
评估脚本 - 支持Teacher和Student模型的全面评估
"""
...
使用示例:
  # 评估教师模型
  python evaluate.py --model_type teacher --model_path ./checkpoints/teacher_best.pt
  
  # 评估学生模型（测试不同模态）
  python evaluate.py --model_type student --model_path ./checkpoints/student_best.pt --test_modalities
  
  # 演示模式（无需模型文件）
  python evaluate.py --demo --model_type teacher
        """,
...
def load_model(args, device):
    ...
    Returns:
        model: 加载的模型
        is_magnitude_model: 是否为频域模型（仅student模型有效）
        checkpoint: 模型checkpoint（用于读取训练配置）
    """
...
def create_dataloader(args, is_magnitude_model=False, checkpoint=None):
    ...
    Args:
        args: 命令行参数
        is_magnitude_model: 是否为频域模型（决定使用哪个数据加载器）
        checkpoint: 模型checkpoint（用于读取训练时的配置）
    """
    # try:
    #     # 🆕 根据模型类型选择数据加载器
    #     if is_magnitude_model:
    #         from new_frame.data_loader import get_magnitude_dataloaders as get_dataloaders
    #         logger.info(f"Using magnitude dataloader for frequency-domain model")
    #     else:
    #         from new_frame.data_loader import get_dataloaders
    #         logger.info(f"Using time-domain dataloader")
    #
    ...
    try:
        ...
        # # 🆕 从checkpoint读取训练时的配置（关键！）
        # if checkpoint and 'config' in checkpoint:
        #     saved_config = checkpoint['config']
        #     # 使用训练时的seq_len
        #     if 'seq_len' in saved_config:
        #         config.window_size = saved_config['seq_len']
        #         logger.info(f"Using seq_len={config.window_size} from checkpoint (training config)")
        #     else:
        #         logger.warning(f"seq_len not found in checkpoint, using default window_size={config.window_size}")
        # else:
        #     logger.warning(f"No checkpoint config available, using default window_size={config.window_size}")
        #
        # 使用真实数据
        ...
        # else:
        #     logger.warning("No normalization stats in checkpoint, using newly computed stats")
        #
        ...
        # 回退到模拟数据
        logger.warning("⚠️ Using dummy dataloader - results are not meaningful!")
...
#         logger.info(f"Testing modality: {mask_name}")
#
#         all_preds = []
#         all_labels = []
#
#         with torch.no_grad():
#             for batch in tqdm(dataloader, desc=f'Eval {mask_name}'):
#                 # 处理字典格式的batch（来自MultiModalTimeSeriesDataset）
#                 if isinstance(batch, dict):
#                     pressure = batch['pressure'].to(device)
#                     vibration = batch['vibration'].to(device)
...
#                 fixed_mask = torch.tensor([mask] * batch_size)
#
#                 # 前向传播
#                 if isinstance(model, StudentModelMagnitudeSDD):
#                     global_logits, _ = model(pressure, vibration, freq_masks=None, mode='test', fixed_mask=fixed_mask,
#     enable_pvnfm_inference=use_pvnfm_inference,
#     pvnfm_inference_weight=pvnfm_inference_weight)
#                     logits = global_logits
#                 else:
#                     logits = model(pressure, vibration, mode='test', fixed_mask=fixed_mask,
#     enable_pvnfm_inference=use_pvnfm_inference,
#     pvnfm_inference_weight=pvnfm_inference_weight)
#                 preds = logits.argmax(dim=1)
#
...
def evaluate_student_modalities(model, dataloader, device,
                                use_pvnfm_inference=False,
                                pvnfm_inference_weight=1.0):
    """评估学生模型的不同模态组合，并用与验证一致的logits融合方式计算Mixed"""
...
def main():
    ...
    pvnfm_inference_weight=args.pvnfm_inference_weight)
            ...
            # 计算混合准确率（Mixed Accuracy）
            # 混合准确率 = (P_only + V_only + Complete) / 3
            # mixed_accuracy = (
            #     results['P_only']['accuracy'] +
            #     results['V_only']['accuracy'] +
            #     results['Complete']['accuracy']
            # ) / 3.0
