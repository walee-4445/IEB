...
# def get_dataloaders_presplit(config, batch_size=None, mode='student',
#                              use_fft=False):
#     """
#     ✅ 使用预划分数据（无泄露风险）
#
...
#     print("=" * 60 + "\n")
#
#     return train_loader, val_loader, test_loader, None  # label_encoder=None
#
def get_dataloaders_presplit(config, batch_size=None, mode='student', use_fft=False, seed=42):  # 🆕 添加FFT开关
    ...
    train_dataset = MultiModalTimeSeriesDataset(
        ...
        mean=None,  # 已归一化
        std=None,
        compute_mag_stats=use_fft,  # 🆕 如果使用FFT，计算幅度谱统计
        use_fft=use_fft,  # 🆕 启用FFT,
        seed=seed  # ✅ 传递seed
    )
    ...
    test_dataset = MultiModalTimeSeriesDataset(
        ...
        mean=None,
        std=None,
        compute_mag_stats=False,  # 测试集不计算统计
        use_fft=use_fft  # 🆕 启用FFT
    )
...
def get_dataloaders(config, batch_size=None, mode='teacher'):
    ...
    Args:
        ...
        batch_size: 批次大小，如果为None则根据mode自动选择
        mode: 'teacher', 'student', 'nfm' - 用于选择默认batch_size

    Returns:
        train_loader, val_loader, test_loader, label_encoder
    """
    # 1. 准备数据集（使用datapre的prepare_dataset）
    ...
    # - 分离压力和振动通道
    # - 应用归一化
    # - 返回字典格式：{'pressure': (W,1), 'vibration': (W,1), 'label': scalar}
    train_dataset = MultiModalTimeSeriesDataset(
        X=data['train']['x'],
        y=data['train']['y'],
        mean=data['norm']['mean'],
        std=data['norm']['std'],  # 🆕 计算幅度谱统计
    ...
    val_dataset = MultiModalTimeSeriesDataset(
        ...
        mean=data['norm']['mean'],
        std=data['norm']['std']
        ,compute_mag_stats=False  # 🆕 计算幅度谱统
    )
    val_dataset.set_mag_stats(
        train_dataset.mag_mean,
        train_dataset.mag_std
    ...
    def worker_init_fn(worker_id):
        """
        为每个DataLoader worker设置独立但确定的随机种子
        确保多进程数据加载的可复现性
        """
...
def get_magnitude_dataloaders(config, batch_size=None, mode='student'):
    ...
    # 4. 创建PyTorch Dataset（幅度谱版本）
    # - 在数据加载阶段进行FFT
    # - 仅保留幅度谱
    # - 返回字典格式：{'pressure': (F,1), 'vibration': (F,1), 'label': scalar}
    # 🆕 训练集：计算幅度谱统计
    ...
    # DataLoader输出格式：
    # batch = {
    #     'pressure': (B, F, 1),   # B=32, F=2561 (幅度谱)
    #     'vibration': (B, F, 1),
    #     'label': (B,)
    # }
...
# import torch
# from torch.utils.data import DataLoader
# from datapre.data_prep import prepare_dataset, check_numpy_dataset
# from datapre.dataset import MultiModalTimeSeriesDataset
#
#
...
#     train_loader = DataLoader(
#         train_dataset,
#         batch_size=config.teacher_batch_size,  # 或根据需要选择batch_size
#         shuffle=True,
#         num_workers=config.num_workers,
#         pin_memory=True if config.device == 'cuda' else False
#     )
